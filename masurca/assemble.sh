...
CONFIG_PATH="/lustre04/scratch/pip17/r2_ip/configuration_ca2.txt"
CMD_PATH="/cvmfs/soft.computecanada.ca/easybuild/software/2017/avx512/Compiler/gcc7.3/masurca/3.3.0/bin/masurca"
set -o pipefail
...
#}
export PATH="/cvmfs/soft.computecanada.ca/easybuild/software/2017/avx512/Compiler/gcc7.3/masurca/3.3.0/bin/../CA8/Linux-amd64/bin:/cvmfs/soft.computecanada.ca/easybuild/software/2017/avx512/Compiler/gcc7.3/masurca/3.3.0/bin:$PATH"
save PATH
export PERL5LIB=/cvmfs/soft.computecanada.ca/easybuild/software/2017/avx512/Compiler/gcc7.3/masurca/3.3.0/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=16
save NUM_THREADS
log 'Processing pe library reads'
rm -rf meanAndStdevByPrefix.pe.txt
echo 'pe 250 38' >> meanAndStdevByPrefix.pe.txt
rename_filter_fastq 'pe' <(exec expand_fastq '/home/pip17/scratch/r2_ip/SRR3177744_1.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/home/pip17/scratch/r2_ip/SRR3177744_2.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'pe.renamed.fastq'
...
JF_SIZE=`ls -l *.fastq | awk '{n+=$5}END{s=int(n/50); if(s>100000000)printf "%.0f",s;else print "100000000";}'`
save JF_SIZE
...
quorum_error_correct_reads  -q $((MIN_Q_CHAR + 40)) --contaminant=/cvmfs/soft.computecanada.ca/easybuild/software/2017/avx512/Compiler/gcc7.3/masurca/3.3.0/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 16 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq --no-discard -o pe.cor.tmp --verbose 1>quorum.err 2>&1 && mv pe.cor.tmp.fa pe.cor.fa || fail Error correction of PE reads failed. Check pe.cor.log.
...
grep --text -A 1 '^>pe' pe.linking.fa | grep --text -v '^\-\-' | sample_mate_pairs.pl $MAX_LINKING_MATES $NUM_LINKING_MATES 1 > pe.tmp && error_corrected2frg pe 250 38 2000000000 pe.tmp > pe.linking.frg.tmp && rm pe.tmp && mv pe.linking.frg.tmp pe.linking.frg
...
runCA -s runCA.spec stopAfter=initialStoreBuilding -p genome -d CA  doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   pe.linking.frg    1> runCA0.out 2>&1
...
runCA -s runCA.spec stopAfter=consensusAfterUnitigger -p genome -d CA  doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   pe.linking.frg    1> runCA1.out 2>&1
...
runCA -s runCA.spec stopAfter=consensusAfterUnitigger -p genome -d CA  doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 superReadSequences_shr.frg   pe.linking.frg    1> runCA2.out 2>&1 
log "Recomputing A-stat for super-reads"
recompute_astat_superreads_CA8.sh genome CA $PE_AVG_READ_LENGTH work1/readPlacementsInSuperReads.final.read.superRead.offset.ori.txt superReadSequences_shr.frg
fi

runCA -s runCA.spec  -p genome -d CA doFragmentCorrection=0 doOverlapBasedTrimming=0 doExtendClearRanges=0 ovlMerSize=30 1>runCA3.out 2>&1
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 100000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'pe.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 16 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
